# ==============================================================================
# Flash2Ride - Linter Fix & Poster-Accurate Help & Support Screen (Section 6)
# 1. Fixes string interpolation info in language_screen.dart
# 2. Creates lib\views\support\help_support_screen.dart
# 3. Links Help & Support in Drawer menu & runs flutter analyze
# ==============================================================================

Write-Host "1. Fixing string interpolation in language_screen.dart..." -ForegroundColor Cyan

$langFile = "lib\views\profile\language_screen.dart"
if (Test-Path $langFile) {
    $langText = [System.IO.File]::ReadAllText($langFile, [System.Text.Encoding]::UTF8)
    $langText = $langText.Replace("Text('(' + lang['native']! + ')')", "Text('(${lang['native']})')")
    $langText = $langText.Replace("'(' + lang['native']! + ')'", "'(${lang['native']})'")
    [System.IO.File]::WriteAllText($langFile, $langText, [System.Text.Encoding]::UTF8)
    Write-Host "  -> language_screen.dart interpolation warning resolved!" -ForegroundColor Green
}

Write-Host "`n2. Creating Help & Support Screen..." -ForegroundColor Cyan

$supportDir = "lib\views\support"
...
[System.IO.File]::WriteAllText("lib\views\support\help_support_screen.dart", $supportCode, [System.Text.Encoding]::UTF8)
Write-Host "  -> Help & Support screen created at lib\views\support\help_support_screen.dart!" -ForegroundColor Green

Write-Host "`n3. Linking Help & Support in Drawer menu..." -ForegroundColor Cyan
...
foreach ($hf in $homeFiles) {
    if (Test-Path $hf) {
        ...
        if ($text -notmatch "help_support_screen\.dart") {
            $text = "import '../../views/support/help_support_screen.dart';`n" + $text
            $modified = $true
        }
        
        # Link Help & Support in Drawer
...
"@
                    ...
                    $modified = $true
                    Write-Host "  -> Successfully linked Drawer 'Help & Support' to HelpSupportScreen in $($hf)!" -ForegroundColor Green
                }
            }
        }
