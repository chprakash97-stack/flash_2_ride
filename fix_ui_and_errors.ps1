...
Write-Host "FIXING PRE-SELECTED MENU, RUPEE SYMBOLS & CONTEXT ERRORS..." -ForegroundColor Green
Write-Host "==========================================================" -ForegroundColor Cyan

$projectRoot = $PWD

# 1. lib\screens\home\home_screen.dart లో మెనూను నార్మల్ గా మార్చడం (ఆటో సెలెక్ట్ లుక్ తొలగించడం)
$homeFile = "$projectRoot\lib\screens\home\home_screen.dart"
if (Test-Path $homeFile) {
    $c = Get-Content $homeFile -Raw -Encoding UTF8

    # బ్లూ కలర్ మరియు సబ్ టైటిల్ తీసేసి సాధారణ లిస్ట్ టైల్ పెట్టడం
    $normalTile = @"
ListTile(
...
"@
    ...
    # పాత బ్లూ స్టైల్ ను రీప్లేస్ చేయడం
    $c = [System.Text.RegularExpressions.Regex]::Replace(
        $c,
        "ListTile\([^)]*title:\s*const\s*Text\('Flash Wallet'[^)]*\)[^)]*onTap:\s*\(\)\s*\{[\s\S]*?\}\s*,\s*\),",
        $normalTile
    )
    [System.IO.File]::WriteAllText($homeFile, $c, [System.Text.Encoding]::UTF8)
    Write-Host "1. Fixed Drawer Menu: Flash Wallet is now normalized (not pre-selected)." -ForegroundColor Green
}

# 2. lib\screens\features\wallet_screen.dart లో యూనికోడ్ రూపాయి గుర్తు (\u20B9) & బాటమ్ షీట్ ఎర్రర్ ఫిక్స్
...
[System.IO.File]::WriteAllText($walletFile, $cleanWalletCode, [System.Text.Encoding]::UTF8)
Write-Host "2. Fixed WalletScreen: Rupee symbols changed to \u20B9 & BottomSheet stateful errors resolved." -ForegroundColor Green

Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "ALL ISSUES RESOLVED! Press 'R' in flutter run terminal." -ForegroundColor Green
Write-Host "==========================================================" -ForegroundColor Cyan