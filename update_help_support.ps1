# ==============================================================================
# Flash2Ride - Poster-Accurate Help & Support Screen Setup
# 1. Cleans character encoding in Payment Methods (â€¢ and â‚¹)
# 2. Creates lib\views\profile\support_screen.dart matching Poster Section 7
# 3. Connects in Routes and Drawer Menu (Three Dots)
# ==============================================================================

Write-Host "1. Cleaning character encoding in Payment Methods..." -ForegroundColor Cyan
$pmFile = "lib\views\payment\payment_methods_screen.dart"
if (Test-Path $pmFile) {
    $pm = [System.IO.File]::ReadAllText($pmFile, [System.Text.Encoding]::UTF8)
    $pm = $pm.Replace("â€¢â€¢â€¢â€¢ â€¢â€¢â€¢â€¢ â€¢â€¢â€¢â€¢", "**** **** ****")
    $pm = $pm.Replace("â€¢", "-")
    $pm = $pm.Replace("â‚¹", "Rs. ")
    $pm = $pm.Replace("•••• •••• ••••", "**** **** ****")
    $pm = $pm.Replace("•", "-")
    [System.IO.File]::WriteAllText($pmFile, $pm, [System.Text.Encoding]::UTF8)
    Write-Host "  -> Cleaned character display in Payment Methods!" -ForegroundColor Green
}

Write-Host "`n2. Creating Poster-Accurate Help & Support Screen..." -ForegroundColor Cyan

$supportDir = "lib\views\profile"
...
[System.IO.File]::WriteAllText("lib\views\profile\support_screen.dart", $supportCode, [System.Text.Encoding]::UTF8)
Write-Host "[1/3] Help & Support Screen created at lib\views\profile\support_screen.dart!" -ForegroundColor Green

Write-Host "`n3. Registering in Routes & Linking in Drawer..." -ForegroundColor Cyan

$routesFile = "lib\core\config\routes.dart"
if (Test-Path $routesFile) {
    $rText = [System.IO.File]::ReadAllText($routesFile, [System.Text.Encoding]::UTF8)
    if ($rText -notmatch "support_screen\.dart") {
        $rText = "import '../../views/profile/support_screen.dart';`n" + $rText
    }
    if ($rText -notmatch "helpSupport = '/help-support'") {
        $rText = $rText -replace "(static const String paymentMethods = '/payment-methods';)", "`$1`n  static const String helpSupport = '/help-support';"
    }
    if ($rText -notmatch "case helpSupport:") {
        $rText = $rText -replace "(case paymentMethods:\s*return _buildRoute\(settings, PaymentMethodsScreen\(data: args\)\);)", "`$1`n      case helpSupport:`n        return _buildRoute(settings, SupportScreen(data: args));"
    }
    [System.IO.File]::WriteAllText($routesFile, $rText, [System.Text.Encoding]::UTF8)
    Write-Host "  -> Route /help-support registered in routes.dart!" -ForegroundColor Green
}
...
foreach ($hf in $homeFiles) {
    if (Test-Path $hf) {
        ...
        if ($text -notmatch "support_screen\.dart") {
            $importStr = "import '../../views/profile/support_screen.dart';"
            if ($hf -match "views[\\/]home") {
                $importStr = "import '../profile/support_screen.dart';"
            }
            $text = "$importStr`n" + $text
            $modified = $true
            Write-Host "  -> Added SupportScreen import in $($hf)" -ForegroundColor Yellow
        }
        
        # Link in Drawer if Help & Support exists or add it
        $hsIdx = $text.IndexOf("Help & Support")
        if ($hsIdx -gt 0) {
            $otIdx = $text.IndexOf("onTap:", $hsIdx)
            if ($otIdx -gt 0 -and ($otIdx - $hsIdx) -lt 300) {
...
"@
                    ...
                    $modified = $true
                    Write-Host "  -> Successfully updated Drawer 'Help & Support' onTap!" -ForegroundColor Green
                }
            }
        } else {
            # Insert before Settings in _buildDrawer
...
"@
                        $text = $text.Substring(0, $ltStart) + $hsTile + $text.Substring($ltStart)
                        $modified = $true
                        Write-Host "  -> Successfully added 'Help & Support' to Drawer menu!" -ForegroundColor Green
                    }
                }
            }
        }
