...
Write-Host "FORCE APPLYING MAP PIN PICKER SCREEN (SECTION 2 - SCREEN 3)..." -ForegroundColor Green
Write-Host "==========================================================" -ForegroundColor Cyan

$projectRoot = $PWD

# -------------------------------------------------------------
# 1. Map Pin Picker Screen (Section 2 - Screen 3) రాయడం
# -------------------------------------------------------------
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        ...
                        // ఇక్కడి Confirm నొక్కినప్పుడు మాత్రమే తర్వాతి స్క్రీన్ అయిన "చూసింగ్ రైడ్ టైప్స్" కి వెళ్లడం
                        Navigator.pushNamed(
                          context,
...
# -------------------------------------------------------------
# 2. Location Search Screen ను నేరుగా MapPinPickerScreen కి కనెక్ట్ చేస్తూ రాయడం
# -------------------------------------------------------------
$searchFiles = @(
...
class _LocationSearchScreenState extends State<LocationSearchScreen> {
  ...
  List<LocationModel> _allPlaces = [];
  List<LocationModel> _filteredPlaces = [];

  @override
  void initState() {
    super.initState();
    _allPlaces = LocationService.getPopularNellorePlaces();
  ...
  // డెస్టినేషన్ ఎంచుకోగానే నేరుగా Map Pin Picker Screen కి వెళ్లేలా చేయడం
  void _selectDestination(LocationModel place) {
    Navigator.push(
      context,
      MaterialPageRoute(
        ...
        children: [
          Container(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    const Icon(Icons.my_location, color: AppColors.success, size: 22),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ActionChip(
                  avatar: const Icon(Icons.home, size: 16, color: AppColors.primary),
                ...
                ActionChip(
                  avatar: const Icon(Icons.work, size: 16, color: AppColors.info),
                  label: const Text('Work'),
                  backgroundColor: AppColors.surface,
                  onPressed: () => _selectDestination(_allPlaces),
                ),
                const SizedBox(width: 8),
                ActionChip(
                  avatar: const Icon(Icons.map, size: 16, color: AppColors.secondary),
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text(place.address, style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
          ...
          // పోస్టర్ లోని Bottom Confirm బటన్ (Section 2 Screen 2)
          Container(
            padding: const EdgeInsets.all(16),
            decoration: const BoxDecoration(
              color: Colors.white,
...
foreach ($sf in $searchFiles) {
    $parentDir = Split-Path $sf -Parent
    if (Test-Path $parentDir) {
        [System.IO.File]::WriteAllText($sf, $searchCode, [System.Text.Encoding]::UTF8)
        Write-Host "Updated LocationSearchScreen at: $sf" -ForegroundColor Green
    }
}
...
Write-Host "MAP PIN PICKER IS NOW FULLY CONNECTED IN THE FLOW!" -ForegroundColor Green
