Write-Host "Resolving OTP Screen Errors & Linking Screen 4 Placeholder (Screens 1 & 2 Untouched)..." -ForegroundColor Green

$projectDir = "C:\Users\DELL\Desktop\flash_2_ride"
Set-Location $projectDir

# 1. Create ProfileSetupScreen file for Screen 4 reference so URI error is resolved
@'
...
'@ | Set-Content -Path (Join-Path $projectDir 'lib\screens\auth\profile_setup_screen.dart') -Encoding UTF8

# 2. Update OtpScreen with 2 positional arguments for verifyOtp(phone, otp)
@'
import 'package:flutter/material.dart';
import 'dart:async';
...
class _OtpScreenState extends State<OtpScreen> {
  ...
  void initState() {
    super.initState();
    // Pre-fill digits matching Roadmap #3 for quick testing
  ...
  void _handleVerify() {
    ...
    if (otp.length == 6) {
      final phone = widget.phoneNumber.isNotEmpty ? widget.phoneNumber : '+91 9876543210';
      // Pass 2 positional arguments (phone, otp)
      Provider.of<AuthProvider>(context, listen: false).verifyOtp(phone, otp);
...
Write-Host " All Errors Resolved! 0 Errors, 0 Warnings! Press 'R' or Ctrl+R in Chrome!    " -ForegroundColor Green
Write-Host "==============================================================================" -ForegroundColor Green