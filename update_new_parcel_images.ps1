...
Write-Host "UPDATING TO NEW 3D PARCEL MODELS (8.33.58 PM)..." -ForegroundColor Green
Write-Host "==========================================================" -ForegroundColor Cyan

$projectRoot = $PWD
Add-Type -AssemblyName System.Drawing

# 1. కొత్త WhatsApp ఇమేజ్ (8.33.58 PM) ను గుర్తించడం
$searchDirs = @(
...
foreach ($dir in $searchDirs) {
    if (Test-Path $dir) {
        $found = Get-ChildItem -Path $dir -Filter "*8.33.58*" -File -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($found) {
...
# ఒకవేళ దొరకకపోతే లేటెస్ట్ WhatsApp Image ను తీసుకోవడం
if (-not $srcImgPath) {
    $found = Get-ChildItem -Path @("$env:USERPROFILE\Downloads", "$env:USERPROFILE\Desktop") -Filter "WhatsApp Image*.jp*g" -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if ($found) {
        $srcImgPath = $found.FullName
    }
}
...
if ($srcImgPath) {
    Write-Host "Found new model image at: $srcImgPath" -ForegroundColor Green
    ...
    # రౌండ్ గ్రాఫిక్ బాక్స్‌కు పక్కాగా సరిపోయేలా క్రాపింగ్
    $cropW = [int]($cw * 0.90)
    $cropH = [int]($ch * 0.90)
    $padX  = [int]($cw * 0.05)
    $padY  = [int]($ch * 0.05)
    ...
    foreach ($item in $items) {
        ...
        $cropped.Dispose()
        Write-Host "Updated new model: $($item.File)" -ForegroundColor Cyan
    }
    $bmp.Dispose()
} else {
    Write-Host "NOTE: Image file not located automatically. Please ensure WhatsApp image is in Downloads or Desktop." -ForegroundColor Yellow
}

# 2. స్క్రీన్ కోడ్ లో ఇమేజ్ సైజు 78x78 కి పెంచి బాక్సులకు నిండుగా సెట్ చేయడం
$parcelCode = @'
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // కొత్త మోడల్ బొమ్మలతో 78x78 సైజు కార్డ్స్
                    GridView.builder(
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                        crossAxisCount: 3,
                        childAspectRatio: 0.80, // ఫుల్ సైజ్ బొమ్మ సరిపోయేలా స్పేస్
                        crossAxisSpacing: 12,
                        mainAxisSpacing: 12,
                      ),
                            ...
                            child: Column(
                              ...
                              children: [
                                Stack(
                                  ...
                                  children: [
                                    // కొత్త 3D బొమ్మ సైజును 78x78 కి పెంచి నిండుగా కనిపించేలా చేయడం
                                    SizedBox(
                                      height: 78,
                                      width: 78,
                                      child: ClipRRect(
...
foreach ($f in $filesToUpdate) {
    if (Test-Path (Split-Path $f)) {
        ...
        Write-Host "Updated with full-sized 78x78 models in $f" -ForegroundColor Green
    }
}

Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "SUCCESS! New 3D models updated. Press 'R' to refresh!" -ForegroundColor Green
Write-Host "==========================================================" -ForegroundColor Cyan