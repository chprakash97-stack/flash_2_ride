Write-Host "Restoring Full Uncut Vehicles (Car, Auto, Bike) and Balancing Login Form..." -ForegroundColor Green
...
# 1. Restore the original uncropped full vehicle image
$searchDirs = @(
...
foreach ($dir in $searchDirs) {
    if (Test-Path $dir) {
        $m = Get-ChildItem -Path $dir -File -Recurse -Depth 2 -ErrorAction SilentlyContinue | 
             Where-Object { $_.Name -like "*WhatsApp*2026-09-10*" -and $_.Name -notmatch "header" } | 
...
if ($found) {
    Write-Host "Found Original Full Graphic: $found" -ForegroundColor Cyan
    Copy-Item -Path $found -Destination $destAsset -Force
    Copy-Item -Path $found -Destination $destWeb -Force
    Write-Host "Restored full uncut vehicles successfully!" -ForegroundColor Green
}

# 2. Update LoginScreen: All Vehicles 100% Visible + Beautifully Positioned Form + 0 Errors!
@'
...
Write-Host " All Vehicles Restored 100%! Perfectly Balanced! Press Ctrl+R in Chrome!      " -ForegroundColor Green
Write-Host "==============================================================================" -ForegroundColor Green