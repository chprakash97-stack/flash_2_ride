Write-Host "Cropping 4 Vehicles from Image 2 & Building Full Official Home Screen Demo (Image 3)..." -ForegroundColor Green
...
# 1. Find the 4-Vehicles Image (Image 2)
$searchDirs = @(
...
$recent = Get-ChildItem -Path $searchDirs -File -Recurse -Depth 2 -ErrorAction SilentlyContinue | 
          Where-Object { ($_.Extension -match "\.(png|jpg|jpeg|webp)$") -and $_.Name -notmatch "splash|login|test|header" -and $_.LastWriteTime -gt (Get-Date).AddHours(-6) } | 
...
foreach ($f in $recent) {
    try {
        ...
        $img.Dispose()
        # 4-Vehicles grid is typically around 1.3 to 1.7 aspect ratio
        if ($ratio -gt 1.2 -and $ratio -lt 1.9) {
            $foundVehicles = $f.FullName
            break
        }
    } catch {}
}

if (-not $foundVehicles) {
    Write-Host "Opening file selector: Please click on the 4-Vehicles image (Image 2)..." -ForegroundColor Yellow
    $dialog = New-Object System.Windows.Forms.OpenFileDialog
    $dialog.InitialDirectory = "$env:USERPROFILE\Downloads"
    $dialog.Filter = "Image Files (*.png;*.jpeg;*.jpg)|*.png;*.jpeg;*.jpg"
    $dialog.Title = "Select Image 2 (4 Vehicles: Bike, Auto, Cab, Parcel)"
    $form = New-Object System.Windows.Forms.Form
    $form.TopMost = $true
    if ($dialog.ShowDialog($form) -eq [System.Windows.Forms.DialogResult]::OK) {
        $foundVehicles = $dialog.FileName
    }
}

if ($foundVehicles) {
    Write-Host "Found 4-Vehicles Image: $foundVehicles" -ForegroundColor Cyan
    $src = [System.Drawing.Bitmap]::FromFile($foundVehicles)
    $w = $src.Width
    $h = $src.Height

    # 1. Bike (Top-Left quadrant - exclude bottom label)
    $rectBike = New-Object System.Drawing.Rectangle([int]($w * 0.05), [int]($h * 0.03), [int]($w * 0.42), [int]($h * 0.38))
    $bmpBike = $src.Clone($rectBike, $src.PixelFormat)
    $bmpBike.Save((Join-Path $assetsDir "vehicle_bike.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpBike.Save((Join-Path $webDir "vehicle_bike.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpBike.Dispose()

    # 2. Auto (Top-Right quadrant)
    $rectAuto = New-Object System.Drawing.Rectangle([int]($w * 0.55), [int]($h * 0.03), [int]($w * 0.40), [int]($h * 0.38))
    $bmpAuto = $src.Clone($rectAuto, $src.PixelFormat)
    $bmpAuto.Save((Join-Path $assetsDir "vehicle_auto.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpAuto.Save((Join-Path $webDir "vehicle_auto.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpAuto.Dispose()

    # 3. Cab (Bottom-Left quadrant)
    $rectCab = New-Object System.Drawing.Rectangle([int]($w * 0.03), [int]($h * 0.54), [int]($w * 0.44), [int]($h * 0.35))
    $bmpCab = $src.Clone($rectCab, $src.PixelFormat)
    $bmpCab.Save((Join-Path $assetsDir "vehicle_cab.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpCab.Save((Join-Path $webDir "vehicle_cab.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpCab.Dispose()

    # 4. Parcel (Bottom-Right quadrant)
    $rectParcel = New-Object System.Drawing.Rectangle([int]($w * 0.56), [int]($h * 0.54), [int]($w * 0.40), [int]($h * 0.35))
    $bmpParcel = $src.Clone($rectParcel, $src.PixelFormat)
    $bmpParcel.Save((Join-Path $assetsDir "vehicle_parcel.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpParcel.Save((Join-Path $webDir "vehicle_parcel.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmpParcel.Dispose()

    $src.Dispose()
    Write-Host "4 Vehicles Cropped & Saved to Assets successfully!" -ForegroundColor Green
}

# 2. Update ONLY lib/screens/home/home_screen.dart matching Image 3 Demo (Screens 1 to 4 are 100% UNTOUCHED!)
@'
      ...
      child: ListView(
        ...
        children: [
          UserAccountsDrawerHeader(
            decoration: const BoxDecoration(color: Color(0xFF0058FF)),
...
Write-Host " Official Home Screen Demo (Image 3) Applied! Press 'R' or Ctrl+R in Chrome!  " -ForegroundColor Green
Write-Host "==============================================================================" -ForegroundColor Green