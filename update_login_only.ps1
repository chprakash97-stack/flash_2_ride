Write-Host "Updating ONLY Screen 2 (Login Screen) with WhatsApp Image (12.01.28 AM)..." -ForegroundColor Green
...
# 1. Search for the 12.01.28 AM WhatsApp Image
$searchDirs = @(
...
foreach ($dir in $searchDirs) {
    if (Test-Path $dir) {
        $m = Get-ChildItem -Path $dir -File -Recurse -Depth 2 -ErrorAction SilentlyContinue | 
             Where-Object { $_.Name -like "*12.01.28*" -or $_.Name -like "*12_01_28*" -or ($_.Name -like "*WhatsApp*2026-09-10*") } | 
...
if ($found) {
    Write-Host "FOUND LOGIN IMAGE:" -ForegroundColor Green
    ...
    Write-Host "Auto-search couldn't find file. Opening selector to click 12.01.28 AM image..." -ForegroundColor Yellow
    Add-Type -AssemblyName System.Windows.Forms
    $dialog = New-Object System.Windows.Forms.OpenFileDialog
    $dialog.InitialDirectory = "$env:USERPROFILE\Downloads"
    $dialog.Filter = "Image Files (*.jpeg;*.jpg;*.png)|*.jpeg;*.jpg;*.png"
    $dialog.Title = "Select WhatsApp Image (12.01.28 AM)"
...
# 2. Update ONLY lib/screens/auth/login_screen.dart with your exact Image & Active Buttons!
@'
          ...
          SafeArea(
            child: Column(
              children: [
                ...
                // Activated Mobile Number Input Box (Editable)
                Padding(
                ...
                // Activated Login Button (Click to navigate to OTP Screen!)
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 24.0),
                  child: SizedBox(
                    width: double.infinity,
                ...
                // Activated Sign Up Button (Clickable)
                Padding(
...
Write-Host " ONLY Login Screen Updated! Splash Screen Untouched. Press 'R' in terminal!   " -ForegroundColor Green
Write-Host "==============================================================================" -ForegroundColor Green